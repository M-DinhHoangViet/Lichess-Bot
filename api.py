...
class API:
    def __init__(self, config: dict) -> None:
        ...
        self.session.headers.update({'User-Agent': f'LichesBOT-Việt/{config["version"]}'})

        account = self.get_account()
        self.username: str = account['username']
        self.user_title: str | None = account.get('title')
        self.user_rated: str | None = account ['rated']
        self.session.headers.update({'User-Agent': f'LichessBOT-Việt/{config["version"]} user:{self.username}'})
