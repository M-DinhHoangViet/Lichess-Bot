...
class API:
    def __init__(self, config: dict) -> None:
        ...
        self.session.headers.update({'Authorization': f'Bearer {config["token"]}',
                                     'User-Agent': f'Lichess-Bot/{config["version"]}'})
    ...
    def get_account(self) -> dict[str, Any]:
        response = self.session.get(urljoin(self.url, f'/api/account'), timeout=3.0)
    ...
    def get_cloud_eval(self, fen: str, variant: Variant, timeout: int) -> dict[str, Any] | None:
        try:
            response = self.session.get(urljoin(self.url, f'/api/cloud-eval'),
    ...
    def get_event_stream(self, queue: Queue) -> None:
        response = self.session.get(urljoin(self.url, f'/api/stream/event'), stream=True, timeout=9.0)
        for line in filter(None, response.iter_lines()):
    ...
    def get_online_bots_stream(self) -> list[dict[str, Any]]:
        response = self.session.get(urljoin(self.url, f'/api/bot/online'), stream=True, timeout=9.0)
    ...
    def get_token_scopes(self, token: str) -> str:
        response = self.session.post(urljoin(self.url, f'/api/token/test'), data=token, timeout=3.0)
        return response.json()[token]['scopes']

    @retry(retry=retry_if_exception_type((requests.ConnectionError, requests.Timeout)), after=after_log(logger, logging.DEBUG))
    def get_user_status(self, username: str) -> dict[str, Any]:
        response = self.session.get(urljoin(self.url, f'/api/users/status'), params={'ids': username}, timeout=3.0)
        return response.json()[0]

    @retry(retry=retry_if_exception_type((requests.ConnectionError, requests.Timeout)), after=after_log(logger, logging.DEBUG))
    def resign_game(self, game_id: str) -> bool:
        try:
            response = self.session.post(urljoin(self.url, f'/ppi/bot/game/{game_id}/resign'), timeout=3.0)
            response.raise_for_status()
            return True
        except requests.HTTPError as e:
            print(e)
    ...
    def upgrade_account(self) -> bool:
        try:
            response = self.session.post(urljoin(self.url, f'/api/bot/account/upgrade'))
            response.raise_for_status()
            return True
        except requests.HTTPError as e:
            print(e)
