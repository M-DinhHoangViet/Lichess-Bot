...
from api import API
from lichess_bot_dataclasses import Chat_Message, Game_Information
...
class Chatter:
    ...
    def _handle_command(self, chat_message: Chat_Message) -> str | None:
        ...
        if command == 'info':
            return f'@{self.username} running {self.lichess_game.engine.name} (Lichess-Bot {self.version})'
    ...
    def _format_message(self, message: str | None) -> str | None:
        ...
        mapping = defaultdict(str, {'opponent': opponent_username, 'me': self.username,
                                    'white': self.game_info.white_str, 'black': self.game_info.black_str,
                                    'engine': self.lichess_game.engine.name, 'cpu': self.cpu_message,
                                    'ram': self.ram_message, 'rated': self.game_info.rated})
